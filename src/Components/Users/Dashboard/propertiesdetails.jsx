    ...
    const fetchProperty = async () => {
      try {
        ...
        const mapped = {
          ...
          price: p.price, type: p.purpose==="sale"?"sale":"rent",
          propertyType: p.type, bedrooms: p.bedrooms??0, bathrooms: p.bathrooms??0,
          toilets: p.toilets??0, sqft: p.propertySize??0, yearBuilt: p.yearBuilt??"—",
          parking: p.parking??0, description: p.description||"",
          features: Array.isArray(p.features)?p.features:[],
          image: p.images?.[0]||"https://placehold.co/1400x900/e5e7eb/6b7280?text=Property",
          videoUrl: p.videos?.[0]||null,
          agent: { name:p.agent?.name||p.agentName||"Agent",avatar:p.agent?.avatar||p.agentAvatar||"",phone:p.agent?.phone||p.agentPhone||"" },
        };
        setProperty(mapped);
      } catch (err) {
        console.error(err);
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#f8fafc",fontFamily:"Inter,sans-serif",display:"flex" }}>
      ...
      <style>{`
        ...
        .prop-main { margin-left:0!important; }
        @media (min-width:768px) { .prop-main { margin-left:256px!important; } }

        .prop-padding { padding:20px 14px 60px!important; }
        @media (min-width:640px) { .prop-padding { padding:28px 24px 60px!important; } }
        @media (min-width:1024px) { .prop-padding { padding:32px 36px 60px!important; } }

        .prop-layout { grid-template-columns:1fr!important; }
        @media (min-width:900px) { .prop-layout { grid-template-columns:1fr 300px!important; } }

        .prop-media { height:260px!important; }
        @media (min-width:480px) { .prop-media { height:320px!important; } }
        @media (min-width:768px) { .prop-media { height:400px!important; } }
        @media (min-width:1024px) { .prop-media { height:460px!important; } }

        .specs-grid { grid-template-columns:repeat(2,1fr)!important; }
        @media (min-width:480px) { .specs-grid { grid-template-columns:repeat(3,1fr)!important; } }

        .title-row { flex-direction:column!important;align-items:flex-start!important; }
        @media (min-width:600px) { .title-row { flex-direction:row!important;align-items:flex-start!important; } }

        .mobile-topbar { display:flex; }
        @media (min-width:768px) { .mobile-topbar { display:none!important; } }

        .breadcrumb-bar { padding:12px 14px!important; }
        @media (min-width:640px) { .breadcrumb-bar { padding:14px 24px!important; } }
        @media (min-width:1024px) { .breadcrumb-bar { padding:14px 36px!important; } }
      `}</style>

      <Sidebar mobileOpen={mobileOpen} setMobileOpen={setMobileOpen}/>

      <main className="prop-main" style={{ flex:1,minHeight:"100vh",boxSizing:"border-box",width:"100%" }}>
        ...
        {/* Breadcrumb */}
        <div className="breadcrumb-bar" style={{ background:WHITE,borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap" }}>
        ...
        <div className="prop-padding">
          {/* Title row */}
          <div className="title-row" style={{ display:"flex",justifyContent:"space-between",marginBottom:18,gap:12 }}>
            <div>
              ...
              <h1 style={{ fontFamily:"Poppins,sans-serif",fontWeight:800,fontSize:"clamp(1.25rem,4vw,1.75rem)",color:NAVY,margin:"0 0 8px",lineHeight:1.2 }}>{property.title}</h1>
            ...
            <div style={{ textAlign:"right",flexShrink:0 }}>
              <div style={{ fontFamily:"Poppins,sans-serif",fontWeight:800,fontSize:"clamp(1.3rem,4vw,1.9rem)",color:BLUE }}>{formatPrice(property.price)}</div>
          ...
          {/* Main layout */}
          <div className="prop-layout" style={{ display:"grid",gap:24,alignItems:"start" }}>

            {/* LEFT */}
            <div>
              <div className="prop-media" style={{ borderRadius:18,overflow:"hidden",boxShadow:"0 12px 48px rgba(0,0,0,0.13)",marginBottom:24 }}>
                <VideoPlayer videoUrl={property.videoUrl} fallbackImage={property.image} title={property.title}/>
              </div>

              {/* Specs */}
              <div className="specs-grid" style={{ background:WHITE,borderRadius:16,padding:"20px 22px",boxShadow:"0 2px 12px rgba(0,0,0,0.06)",marginBottom:20,display:"grid",gap:16 }}>
                {specs.map(({ icon:Icon, label, value }) => (
                  <div key={label} style={{ display:"flex",alignItems:"center",gap:10 }}>
                    <div style={{ width:38,height:38,borderRadius:10,background:"#f0f4ff",display:"grid",placeItems:"center",flexShrink:0 }}>
                      <Icon size={17} color={BLUE}/>
                    </div>
                    <div>
                      <p style={{ fontSize:10.5,color:"#9ca3af",fontWeight:500,margin:0,textTransform:"uppercase",letterSpacing:"0.5px" }}>{label}</p>
                      <p style={{ fontSize:14,fontWeight:700,color:NAVY,margin:0,fontFamily:"Poppins,sans-serif" }}>{value}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background:WHITE,borderRadius:16,padding:"20px 22px",boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
                ...
                <div style={{ borderRadius:12,overflow:"hidden",border:"1px solid #e5e7eb" }}>
                  <iframe title="Property Location" width="100%" height="240" style={{ border:0,display:"block" }} loading="lazy" allowFullScreen referrerPolicy="no-referrer-when-downgrade"
              ...
              <div style={{ background:WHITE,borderRadius:18,boxShadow:"0 8px 36px rgba(0,0,0,0.1)",overflow:"hidden" }}>
                <div style={{ background:NAVY,padding:"18px 20px",display:"flex",alignItems:"center",gap:12 }}>
                  <img src={property.agent.avatar} alt={property.agent.name}
                    style={{ width:46,height:46,borderRadius:"50%",objectFit:"cover",border:"2px solid rgba(255,255,255,0.25)",flexShrink:0 }}
                    onError={e=>{ e.target.src=`https://ui-avatars.com/api/?name=${encodeURIComponent(property.agent.name)}&background=1a56db&color=fff&size=46`; }}/>
                  <div>
                    <p style={{ color:"#93c5fd",fontSize:10,fontWeight:600,letterSpacing:1,textTransform:"uppercase",margin:"0 0 2px" }}>Listed by</p>
                    <p style={{ color:WHITE,fontWeight:700,fontSize:14,margin:"0 0 1px",fontFamily:"Poppins,sans-serif" }}>{property.agent.name}</p>
                    <p style={{ color:"rgba(255,255,255,0.6)",fontSize:11.5,margin:0 }}>Verified Agent</p>
                  </div>
                </div>
                <div style={{ padding:"20px 20px" }}>
                  ...
                  <button onClick={()=>setShowModal(true)} style={{ width:"100%",padding:"14px",background:`linear-gradient(135deg,${BLUE} 0%,#1444b8 100%)`,color:WHITE,border:"none",borderRadius:12,fontFamily:"Poppins,sans-serif",fontWeight:700,fontSize:15,cursor:"pointer",boxShadow:"0 8px 24px rgba(26,86,219,0.38)",transition:"all .22s" }}
                  ...
                  <div style={{ marginTop:16,padding:"12px 14px",background:"#f8fafc",borderRadius:10,textAlign:"center",border:"1px solid #f3f4f6" }}>
                    <p style={{ fontSize:10.5,color:"#9ca3af",margin:"0 0 2px",textTransform:"uppercase",letterSpacing:"0.5px",fontWeight:600 }}>Listing Price</p>
                    <p style={{ fontFamily:"Poppins,sans-serif",fontWeight:800,fontSize:"1.25rem",color:BLUE,margin:0 }}>{formatPrice(property.price)}</p>
                    <p style={{ fontSize:11,color:"#9ca3af",margin:"2px 0 0" }}>{property.type==="rent"?"per year":"asking price"}</p>
                  </div>
                </div>
              </div>
            </div>
